...
class StockApp:
    ...
    def __init__(self, root, embedded=False):
        ...
        self.trainer = StockTrainer(model_dir=os.path.join(base_dir, "models"))
    ...
    def _build_param_panel(self, parent):
        ...
        ttk.Label(scroll_frame, text="══════ 数据参数 ══════", font=("Arial", 11, "bold")).grid(
        ...
        freq_combo = ttk.Combobox(scroll_frame, textvariable=self.frequency_var,
                                  values=["d-日K", "w-周K", "m-月K", "5-5分钟", "15-15分钟", "30-30分钟", "60-60分钟"],
                                  state="readonly", width=18)
        ...
        ttk.Button(scroll_frame, text="下载并预处理数据", command=self._on_load_data).grid(
            row=row, column=0, columnspan=2, sticky="ew", padx=2, pady=6)
        row += 1

        ttk.Label(scroll_frame, text="══════ 模型参数 ══════", font=("Arial", 11, "bold")).grid(
        ...
        model_combo.bind("<<ComboboxSelected>>", lambda e: self._on_model_type_change())
        row += 1
        ...
        ttk.Spinbox(scroll_frame, from_=1, to=8, textvariable=self.num_layers_var, width=18).grid(
            row=row, column=1, sticky="w", padx=2, pady=2)
        row += 1
        ...
        ttk.Checkbutton(scroll_frame, text="双向 (LSTM/GRU)", variable=self.bidirectional_var).grid(
        ...
        ttk.Spinbox(scroll_frame, from_=1, to=16, textvariable=self.nhead_var, width=18).grid(
            row=row, column=1, sticky="w", padx=2, pady=2)
        row += 1
        ...
        ttk.Label(scroll_frame, text="══════ 训练参数 ══════", font=("Arial", 11, "bold")).grid(
            row=row, column=0, columnspan=2, sticky="w", pady=(8, 4))
        row += 1

        ttk.Label(scroll_frame, text="训练轮数:").grid(row=row, column=0, sticky="w", padx=2, pady=2)
        self.epochs_var = tk.IntVar(value=100)
        ttk.Spinbox(scroll_frame, from_=1, to=5000, textvariable=self.epochs_var, width=18).grid(
            row=row, column=1, sticky="w", padx=2, pady=2)
        row += 1

        ttk.Label(scroll_frame, text="批次大小:").grid(row=row, column=0, sticky="w", padx=2, pady=2)
        self.batch_size_var = tk.IntVar(value=32)
        ttk.Spinbox(scroll_frame, from_=1, to=512, textvariable=self.batch_size_var, width=18).grid(
            row=row, column=1, sticky="w", padx=2, pady=2)
        row += 1
        ...
        ttk.Spinbox(scroll_frame, from_=0.00001, to=0.1, increment=0.0001,
                     textvariable=self.learning_rate_var, width=18, format="%.6f").grid(
            row=row, column=1, sticky="w", padx=2, pady=2)
        row += 1
        ...
        ttk.Spinbox(scroll_frame, from_=0, to=100, textvariable=self.early_stopping_var, width=18).grid(
            row=row, column=1, sticky="w", padx=2, pady=2)
        row += 1
        ...
        ttk.Label(scroll_frame, text="══════ 模型管理 ══════", font=("Arial", 11, "bold")).grid(
            row=row, column=0, columnspan=2, sticky="w", pady=(8, 4))
        row += 1

        ttk.Label(scroll_frame, text="模型名称:").grid(row=row, column=0, sticky="w", padx=2, pady=2)
        self.model_name_var = tk.StringVar(value="stock_model")
        ttk.Entry(scroll_frame, textvariable=self.model_name_var, width=20).grid(
            row=row, column=1, sticky="w", padx=2, pady=2)
        row += 1

        ttk.Label(scroll_frame, text="已有模型:").grid(row=row, column=0, sticky="w", padx=2, pady=2)
        ...
        ttk.Button(btn_frame, text="保存模型", command=self._on_save_model).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=2)
        ttk.Button(btn_frame, text="加载模型", command=self._on_load_model).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=2)
        ttk.Button(btn_frame, text="删除模型", command=self._on_delete_model).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=2)
        row += 1
        ...
        ttk.Button(scroll_frame, text="刷新模型列表", command=self._refresh_model_list).grid(
    ...
    def _build_result_panel(self, parent):
        ...
        ttk.Label(top_frame, text="运行进度:", font=("Arial", 10, "bold")).pack(side=tk.LEFT)
    ...
    def _plot_data_overview(self):
        ...
        ax.plot(dates[:train_size], close_prices[:train_size], label="训练集", color="blue", linewidth=0.8)
        ax.plot(dates[train_size:], close_prices[train_size:], label="测试集", color="orange", linewidth=0.8)
        ax.set_title(f"{self.stock_code_var.get()} 收盘价走势")
        ax.set_xlabel("日期")
        ax.set_ylabel("价格")
        ax.legend()
    ...
    def _train_thread(self, params):
        try:
            ...
            self._log(f"模型类型: {params['model_type']}, 参数数: {sum(p.numel() for p in self.trainer.model.parameters())}")
            ...
            def epoch_cb(epoch, total_epochs, train_loss, val_loss):
                ...
                if epoch % max(1, total_epochs // 20) == 0 or epoch == total_epochs:
                    self._log(f"Epoch {epoch}/{total_epochs} - train_loss: {train_loss:.6f}, val_loss: {val_loss:.6f}")
    ...
    def _plot_training_result(self):
        ...
        test_dates = self.data_loader.get_test_dates(
            seq_len=self.seq_len_var.get(), train_ratio=self.train_ratio_var.get())

        ax2.plot(test_dates, y_test_actual, label="实际值", color="blue", linewidth=1)
        ax2.plot(test_dates, test_pred_actual, label="预测值", color="red", linewidth=1, linestyle="--")
        ax2.set_title("测试集预测 vs 实际")
        ax2.set_xlabel("日期")
        ax2.set_ylabel("价格")
        ax2.legend()
    ...
    def _predict_thread(self):
        try:
            ...
            mae = np.mean(np.abs(test_pred_actual - y_test_actual))
            rmse = np.sqrt(mse)
            mape = np.mean(np.abs((test_pred_actual - y_test_actual) / y_test_actual)) * 100
    ...
    def _plot_full_prediction(self, train_pred, test_pred, y_train, y_test):
        ...
        ax.axvline(x=train_dates[-1], color="green", linestyle="--", alpha=0.7, label="训练/测试分界")

        ax.set_title(f"{self.stock_code_var.get()} 收盘价预测结果")
        ax.set_xlabel("日期")
        ax.set_ylabel("价格")
        ax.legend()
        ax.grid(True, alpha=0.3)
    ...
    def _on_save_model(self):
        ...
        try:
            ...
            path = self.trainer.save_model(model_name, data_loader=self.data_loader, extra_info=extra)
    ...
    def _on_load_model(self):
        ...
        try:
            ...
            self._log(f"模型 {model_name} 加载成功！")
    ...
    def _on_delete_model(self):
        ...
        if not model_name:
            ...
            return
        if not messagebox.askyesno("确认", f"确定要删除模型 {model_name} 吗？"):
            return
        try:
