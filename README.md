...
└── src/
    ...
    └── favorite_stocks_page.py  # 自选股页面（本地 JSON + 事件到期提醒）
```
...
### 功能
- 本地 JSON 文件（`data/favorites.json`）存储自选股，不依赖外部服务
- 每条自选股记录：股票代码、名称、**买入日期**、**买入价**、**当前价**、**收益(%)**、**备注**、自定义事件列表
- 收益率自动计算：`(当前价 - 买入价) / 买入价 × 100%`
- 「刷新行情」按钮通过 baostock 批量拉取最新收盘价，自动更新当前价与收益
- **自定义事件到期提醒**：每只自选股可挂多个事件（标题 + 到期日 YYYY-MM-DD）
  - 后台线程每 60 秒检查一次，到期日 ≤ 今日时弹框提醒
  - 到期行**红色底重点显示**（红底 + 红字）
  - 同一事件仅弹框一次（避免重复打扰），可点「立即检查到期」手动复查
- 列头点击排序、代码/名称筛选、双击行编辑、导出 CSV

### 字段
| 列名 | 说明 |
|---|---|
| 股票名称 | 手动填写 |
| 代码 | baostock 格式，如 sh.600036 |
| 买入日期 | YYYY-MM-DD |
| 买入价 | 数字 |
| 当前价 | 手动填或「刷新行情」自动获取 |
| 收益(%) | 自动计算 |
...
### 使用
1. 点「添加自选股」填写表单（可附加多个事件），保存
2. 双击表格行或选中后点「编辑」修改
3. 选中行点「删除」移除
4. 点「刷新行情」批量拉取最新收盘价（baostock）
5. 点「立即检查到期」手动触发到期扫描
6. 到期事件所在行自动红色高亮 + 弹框提醒
